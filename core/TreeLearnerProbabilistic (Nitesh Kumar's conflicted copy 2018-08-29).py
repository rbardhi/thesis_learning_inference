...
class TreeLearnerProbabilistic(object):
    EPSILON_SCORE = 1
    MIN_NUMBER_OF_DATAPOINT_AT_LEAF = 3
    NUM_OF_LOOKAHEADS = 1
    NUM_OF_SAMPLES = 10
    
    def __init__(self, inputFile, dcFileName, whichProlog, firstFeatureName):
                ...
                for feature in typeModeConformFeatures:
                    ...
                    for feat in featureString:
                        ...
                        if (relF == True) and (remainingLookAhead > 0):
                            ...
                            scoreAndDistribution = self.scoreFinder.getScore(bigX, bigY, self.targetType, xVar, bigP, numOfExamples)
                            sc = scoreAndDistribution['score']
                            numP = len(bigY)
                            lookaheadFeature = feat
                        if np.isnan(sc) or (len(bigY) < self.MIN_NUMBER_OF_DATAPOINT_AT_LEAF):
                            #score += float('-inf')
                            pass
                        elif feat[0] == '\\':
                            score += sc
    ...
    def learnRules(self):
        ...
        for target in targets:
            ...
            #bigYCross = prologResult[self.targetVariable]
            scoreAndDistribution = self.scoreFinder.getScore([], bigY, self.targetType, [], bigP, numOfExamples)
            sc = float('-inf')
            if len(bigY) > 0:
...
if __name__ == '__main__':
    ...
    f = open(outputFile, 'w')
    obj = TreeLearnerProbabilistic('../data/hepatitisCont.pl', '../data/dcHepatitisCont.pl', 'SWI-prolog', '')
    obj.learnRules()
    obj1 = TranslateToDC()
