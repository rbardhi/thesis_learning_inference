...
logging.basicConfig(level=logging.INFO, filename="Experiment2", filemode="w", format="%(asctime)-15s %(levelname)-8s %(message)s")

experiment2_logger = logging.getLogger('Experiment 2 Logger :')
experiment2_logger_handler = logging.FileHandler("Experiment2", mode="w")
experiment2_logger_handler.setFormatter(logging.Formatter("%(asctime)-15s %(levelname)-8s %(message)s"))
experiment2_logger_handler.setLevel(logging.INFO)
experiment2_logger.addHandler(experiment2_logger_handler)
...
def generateDLTsFromValidation(numOfSamples, validationFolder, prologValidationFile, newTrainFacts, missingFacts, treeOutputFile, tempDcFileName, missingPredicate):
    ...
    for randVariable in missingPredicate:
        ...
        exp1Obj.generatePrologFile(prologValidationFile, validateFacts, targetVariable=randVariable)
        obj = TreeLearnerProbabilistic(prologValidationFile, '', 'SWI-prolog', '', RANDOM_VARIABLE_PREDICATE[DATABASE_NAME])
        obj.learnRules()
        obj1 = TranslateToDC()
        dcRules = []
        ...
        restValidateFacts = []
        tempValidateFacts = []
        for tFacts in validateFacts:
            if randVariable not in tFacts:
                restValidateFacts.append(tFacts)
            else:
                tempValidateFacts.append(tFacts)
    ...
    for predicate in listOfTargetPredicates:
        ...
        if ty == 'continuous':
            ...
            if NUM_OF_ELEMENTS_IN_TEST[predicate] == 0:
                experiment2_logger.info('WRMSE = %f, Number of tests consider = %d' % (acc, NUM_OF_ELEMENTS_IN_TEST[predicate]))
            else:
                ...
                experiment2_logger.info('WRMSE = %f, Number of tests consider = %d' % (wrmse, NUM_OF_ELEMENTS_IN_TEST[predicate]))
        else:
            uniqueLabel = list(set(YPRED[predicate] + YTRUE[predicate]))
            experiment2_logger.info ('Confusion Matrix:')
            experiment2_logger.info(confusion_matrix(YTRUE[predicate], YPRED[predicate], labels=uniqueLabel))
            experiment2_logger.info('Labels: '  + str(uniqueLabel))
            if NUM_OF_ELEMENTS_IN_TEST[predicate] == 0:
                experiment2_logger.info('Accuracy: nan')
                experiment2_logger.info('Area Under ROC: prediction list is empty')
            else:
                experiment2_logger.info('Accuracy: %f'  % accuracy_score(YTRUE[predicate], YPRED[predicate]))
                try:
                    ...
                    experiment2_logger.info('Area Under ROC: %f' % auROC)
                except ValueError:
                    experiment2_logger.info('Only one class present in y_true, Area Under ROC: %f' % 1)
...
def experiment2(numOfSamples, trainingFolder, validationFolder, testFolder, prologTrainingFileName, dcTrainingFile, tempDcFileName, testDcFileName, percentageMissing, prologValidationFile, treeOutputFileTraining, treeOutputFileValidation):
    ...
    for missingPredicate in listOfMissingPredicate:
        ...
        for i in range(0, len(trainFacts)):
            ...
            if missingPredicateName != '':
                ...
                newTrainFacts.append(trainFacts[i])
        [listOfD, listOfM, listOfE, listOfA] = generateDLTsFromValidation(numOfSamples, validationFolder, prologValidationFile, newTrainFacts, missingFacts, treeOutputFileValidation, tempDcFileName, [missingPredicate])
    ...
    #modeList = ['mode','missing','complete']
...
FILE_COUNTER = 0
numOfSamples = 1000
trainingFolder = '/home/nitesh/eclipse-workspace/DCRuleLearningTestBed/data/PKDD/pkdd/Fold1/validate1'
validationFolder = '/home/nitesh/eclipse-workspace/DCRuleLearningTestBed/data/PKDD/pkdd/Fold1/train1'
testFolder = '/home/nitesh/eclipse-workspace/DCRuleLearningTestBed/data/PKDD/pkdd/Fold1/test'
...
percentageMissingList = [50, 40, 30, 20]
...
for j in range(6,11):
    ...
    for i in range(0,1):
        ...
        for precent in percentageMissingList:
            ...
            try:
                experiment2(numOfSamples, trainFoldString, validationFoldString, testFolderString, prologTrainingFileName, dcTrainingFile, tempDcFileName, testDcFileName, percentageMissing, prologValidationFile, treeOutputFileTraining, treeOutputFileValidation)
            except RuntimeError:
                precent = precent - 1
