...
from Experiment1 import RANDOM_VARIABLE_PREDICATE_TYPE, RANDOM_VARIABLE_PREDICATE, DATABASE_NAME, RANDOM_VARIABLE_PREDICATE_RANGE, DECLARATIVE_BIAS, PROLOG_PROGRAM_HEADER, DC_PROGRAM_HEADER, DC_PROGRAM_DATA_DEPENDENT_RELATION
from core.Experiment1 import PROLOG_PROGRAM_HEADER_PROBABILISTIC,\
    DC_PROGRAM_DATA_DEPENDENT_RELATION_PROBABILISTIC

logging.basicConfig(level=logging.INFO, filename="Experiment3", filemode="w", format="%(asctime)-15s %(levelname)-8s %(message)s")
experiment3_logger = logging.getLogger('Experiment 3 Logger :')
experiment3_logger_handler = logging.FileHandler("Experiment3", mode="w")
experiment3_logger_handler.setFormatter(logging.Formatter("%(asctime)-15s %(levelname)-8s %(message)s"))
experiment3_logger_handler.setLevel(logging.INFO)
experiment3_logger.addHandler(experiment3_logger_handler)
...
def findTestAccuracy(numOfSamples, tempDcFileName, testFacts, listOfTargetPredicates, trainFacts, listOfDistributionalFacts, listOfEnumeration, listOfModeOfDistribtion, dcRules, mode):
    ...
    for featName in listOfTargetPredicates:
        ...
        print "Going to sleep for 2 secs fuck off1 ."
        time.sleep(2)
        print "Restarting again :)"
        yapObj = YapPrologInterface(RANDOM_VARIABLE_PREDICATE[DATABASE_NAME])
        print "Going to sleep for 2 secs fuck off2"
        time.sleep(2)
        print "Restarting again :)"
    ...
    for predicate in listOfTargetPredicates:
        ...
        if ty == 'continuous':
            ...
            if NUM_OF_ELEMENTS_IN_TEST[predicate] == 0:
                experiment3_logger.info('WRMSE = %f, Number of tests consider = %d' % (acc, NUM_OF_ELEMENTS_IN_TEST[predicate]))
            else:
                ...
                experiment3_logger.info('WRMSE = %f, Number of tests consider = %d' % (wrmse, NUM_OF_ELEMENTS_IN_TEST[predicate]))
        else:
            uniqueLabel = list(set(YPRED[predicate] + YTRUE[predicate]))
            experiment3_logger.info ('Confusion Matrix:')
            experiment3_logger.info(confusion_matrix(YTRUE[predicate], YPRED[predicate], labels=uniqueLabel))
            experiment3_logger.info('Labels: '  + str(uniqueLabel))
            if NUM_OF_ELEMENTS_IN_TEST[predicate] == 0:
                experiment3_logger.info('Accuracy: nan')
                experiment3_logger.info('Area Under ROC: prediction list is empty')
            else:
                experiment3_logger.info('Accuracy: %f'  % accuracy_score(YTRUE[predicate], YPRED[predicate]))
                try:
                    ...
                    experiment3_logger.info('Area Under ROC: %f' % auROC)
                except ValueError:
                    experiment3_logger.info('Only one class present in y_true, Area Under ROC: %f' % 1)
...
def generateDLTsFromTrain(numOfSamples, newTrainFacts, missingFactsDict, listOfDistributionalFacts, listOfEnumeration, interPrologFileName, interDcFileName, tempDCFileName, treeOutputFileIntermediate, missingPredicate):
    listOfDistributinalFacts = []
    listOfModeOfDistribtion = []
    listOfActualValue = []
    listOfEnumeration = []
    for randVariable in missingPredicate:
        generateDCPrologFile(interPrologFileName, interDcFileName, newTrainFacts, listOfDistributionalFacts, listOfEnumeration, [], [], 'probabilistic', targetVariable=randVariable)
        obj = TreeLearnerProbabilistic(interPrologFileName, interDcFileName, 'SWI-prolog', '', RANDOM_VARIABLE_PREDICATE[DATABASE_NAME])
        obj.learnRules()
        obj1 = TranslateToDC()
        dcRules = []
        obj1.setRandomVariablePredicates(RANDOM_VARIABLE_PREDICATE[DATABASE_NAME])
        with open(treeOutputFileIntermediate, 'w') as f:
            for rule in obj.rules:
                rule = obj1.translate(rule)
                dcRules.append(rule)
                f.write(rule + '\n')
        missingFacts = missingFactsDict[randVariable]
        ...
        generateDCFile(tempDCFileName, newTrainFacts, listOfDistributionalFacts, listOfEnumeration, [], [], dcRules)
        yapObj = YapPrologInterface(RANDOM_VARIABLE_PREDICATE[DATABASE_NAME])
        ...
        yapObj.executeQuery('abort.')
        del yapObj
    return [listOfDistributinalFacts, listOfModeOfDistribtion, listOfEnumeration, listOfActualValue]

def experiment3(numOfSamples, trainingFolder, testFolder, percentageMissing, interPrologFileName, interDcFileName, prologTrainingFileName, dcTrainingFile, testDcFileName, treeOutputFileIntermediate, treeOutputFileTraining):
    countsPerPredicate = dict()
    missingFactsDict = dict()
    dictOfEnumeration = dict()
    dictOfDistributionalFacts = dict()
    for i in listOfMissingPredicate:
        countsPerPredicate[i] = 0
        missingFactsDict[i] = []
        dictOfEnumeration[i] = dict()
        dictOfDistributionalFacts[i] = dict()
        for j in listOfMissingPredicate:
            dictOfEnumeration[i][j] = dict()
            dictOfDistributionalFacts[i][j] = dict()
    ...
    modeList = ['probabilistic', 'missing']
            ...
            if mode == 'probabilistic':
                for j in range(0, numOfIterations):
                    for missingPredicate in listOfMissingPredicate:
                        ...
                        listOfEnumerationFacts = []
                        for i in listOfMissingPredicate:
                            listOfDistributionalFacts = listOfDistributionalFacts + dictOfDistributionalFacts[missingPredicate][i]
                            listOfEnumerationFacts = listOfEnumerationFacts + dictOfDistributionalFacts[missingPredicate][i]
                        [listOfD, listOfM, listOfE, listOfA] = generateDLTsFromTrain(numOfSamples, newTrainFacts, missingFactsDict, listOfDistributionalFacts, listOfEnumerationFacts, interPrologFileName, interDcFileName, treeOutputFileIntermediate, [missingPredicate])
                        for i in listOfMissingPredicate:
                            if i == missingPredicate:
                                pass
                            else:
                                dictOfDistributionalFacts[i][missingPredicate] = listOfD
                                dictOfEnumeration[i][missingPredicate] = listOfE
                for i in listOfMissingPredicate:
                    listOfDistributionalFacts = listOfDistributionalFacts + dictOfDistributionalFacts[missingPredicate][i]
                    listOfEnumerationFacts = listOfEnumerationFacts + dictOfDistributionalFacts[missingPredicate][i]
                generateDCPrologFile(prologTrainingFileName, dcTrainingFile, newTrainFacts, listOfDistributionalFacts, listOfEnumerationFacts, [], background, mode, targetVariable=randVariable)
                f = open(treeOutputFileTraining, 'w')
                obj = TreeLearnerProbabilistic(prologTrainingFileName, dcTrainingFile, 'SWI-prolog', '', RANDOM_VARIABLE_PREDICATE[DATABASE_NAME])
                obj.learnRules()
                obj1 = TranslateToDC()
                dcRules = []
                obj1.setRandomVariablePredicates(RANDOM_VARIABLE_PREDICATE[DATABASE_NAME])
                for rule in obj.rules:
                    ...
                    f.write(rule + '\n')
                f.close()
                findTestAccuracy(numOfSamples, testDcFileName, testFacts, [randVariable], newTrainFacts, listOfDistributionalFacts, listOfEnumerationFacts, [], dcRules, mode)
                ...
                f = open(treeOutputFileTraining, 'w')
                obj = TreeLearnerProbabilistic(prologTrainingFileName, '', 'SWI-prolog', '', RANDOM_VARIABLE_PREDICATE[DATABASE_NAME])
                obj.learnRules()
                obj1 = TranslateToDC()
                dcRules = []
                obj1.setRandomVariablePredicates(RANDOM_VARIABLE_PREDICATE[DATABASE_NAME])
...
interPrologFileName = '../data/FinancialTemp.pl'
dcTrainingFile = '../data/FinancialDC.pl'
interDcFileName = '../temp/FinancialDCTemp.pl'
testDcFileName = '../data/FinancialDCTest.pl'
treeOutputFileTraining = '../data/EnsembleOfDLTs.pl'
treeOutputFileIntermediate = '../data/EnsembleOfDLTsIntermediate.pl'
percentageMissingList = [90, 80, 70, 60, 50, 40, 30, 20]
folder = '/home/nitesh/eclipse-workspace/DCRuleLearningTestBed/data/PKDD/pkdd/'
missPred = [['loanStatus', 'loanAmount', 'freq', 'stdMonthInc', 'stdMonthW', 'avgSumOfInc', 'gender', 'monthlyPayments'], ['monthlyPayments', 'loanAmount', 'freq', 'avgSumOfW', 'stdMonthInc', 'stdMonthW', 'avgNrWith', 'avgSumOfInc', 'avgSalary', 'ratUrbInhab', 'gender', 'loanStatus']]
...
for j in range(1,11):
    ...
    for i in range(0,1):
        ...
        for precent in percentageMissingList:
            ...
            try:
                experiment3(numOfSamples, trainFoldString, testFolderString, percentageMissing, interPrologFileName, interDcFileName, prologTrainingFileName, dcTrainingFile, testDcFileName, treeOutputFileIntermediate, treeOutputFileTraining)
            except RuntimeError:
                precent = precent - 1
