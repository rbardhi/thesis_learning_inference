...
class ScoreFinderProbabilistic(object):
    ...
    def fitGaussainDistribution(self, bigY, bigYCross, bigP, numOfExamples):
        ...
        score = 2*score #score = 2*score+pScore
    ...
    def fitLinearRegressionModel(self, bigX, bigY, bigXCross, bigYCross, xVar, bigP, numOfExamples):
        ...
        for i in range(0, rowSize):
            ...
            data.append(aRow)
        model = self.gaussian.fit(data, bigY, sample_weight=bigP)
        coef = model.coef_
        ...
        score = 2*score-(numCoef*np.log(numOfExamples)) #score = 2*score+pScore-(numCoef*np.log(numOfExamples))
    ...
    def getScore(self, bigX, bigY, targetType, xVar, bigP, numOfExamples, numSamples):
        ...
        if len(bigY) != numOfExamples:
            samplesPresent = True
        for i in range(0, len(bigP)):
            if bigP[i] == 0.0:
                if samplesPresent:
                    pass
                    #numOfExamples = numOfExamples - 1/float(numSamples)
                else:
                    pass
