...
public class TuPrologInterpretationCreator_positive_examples_Test {
	...
	public void checkInterpretation() throws SubstitutionException{
		TuPrologInterpretationCreator_positive_examples tC=new TuPrologInterpretationCreator_positive_examples();
		...
		for(GroundAtom g:i.getGroundAtoms(takes)){
			if(trueAtoms.contains(g)){
				assertTrue(true);
			}
			else{
				assertTrue(false);
		...
		assertEquals(9, i.getGroundAtoms(takes).size(),0.1);
