...
public class TestLinearGParameters {
	...
	@Test
	public void testNrParameters() throws FeatureTypeException, ConjunctionConstructionProblem{
		Max ft1=new Max(new Standard_Conjunction(intelligence,new PosLiteral(takes),new PosLiteral(teaches),new PosLiteral(ability)));
		...
		Regression reg=new Regression(15.0,dep.getContinuousFeatures(),new Double[]{6.0,3.5});
		LinearGParameters lgPars=new LinearGParameters(dep,reg,80.5);
		System.out.println(dep);
		assertEquals(4,lgPars.getNumberOfFreeParameters());
	}
	
	@Test
	public void testParameters() throws FeatureTypeException, ConjunctionConstructionProblem{
		Max ft1=new Max(new Standard_Conjunction(intelligence,new PosLiteral(takes),new PosLiteral(teaches),new PosLiteral(ability)));
		ValueFt ft2=new ValueFt(new Standard_Conjunction(intelligence,new PosLiteral(takes),new PosLiteral(teaches),new PosLiteral(ability)));
		Dependency dep=new Dependency(intelligence,new Feature[]{ft1,ft2});
		Regression reg=new Regression(15.0,dep.getContinuousFeatures(),new Double[]{6.0,3.5});
		...
		assertEquals(15.0,lgPars.getPars().getReg_coeff().getIntercept(),0.01);
		HashMap<Feature,Double> coeffs=new HashMap<Feature,Double>();
		coeffs.put(ft2,3.5);
		coeffs.put(ft1, 6.0);
		
		assertEquals(15.0,lgPars.getPars().getReg_coeff().getIntercept(),0.01);
		assertEquals(coeffs,lgPars.getPars().getReg_coeff().getWeights());
		assertEquals(80.5,lgPars.getPars().getStd(),0.01);
	}
}
