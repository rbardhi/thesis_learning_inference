...
public class TestApplyingRenamingsOnProposedConjunctions {
	...
	@Test
	public void testApplyingAllRenamings1() throws Exception{
		ProposedConjunction testClass=new ProposedConjunction(new PosLiteral(friend), 2);
		ProposedConjunction prop=testClass.extend(new PosLiteral(friend)).extend(new PosLiteral(friend));
		System.out.println(prop);
       /* FeatureGenerator gen=new FeatureGenerator();
        Predicate fr=new BooleanPred("friend",2);
        Atom friendHead=new Atom(fr,new Logvar[]{student,student1});
        Assert.assertEquals(1000,gen.createAllRenamingCombinations(Arrays.asList(new ProposedConjunction[]{testClass}), friendHead).size());
	    List<Conjunction> created_conjunctions=gen.createAllRenamingCombinations(Arrays.asList(new ProposedConjunction[]{testClass}), friendHead);
	    Conjunction first=created_conjunctions.get(0);
	    Assert.assertEquals(first.getAtomList().get(0),first.getAtomList().get(1));
	    System.out.println(created_conjunctions);*/
	}
	
	/*@Test
	public void testApplyingAllRenamings2() throws Exception{
		List<Atom> atoms=new ArrayList<Atom>();
		atoms.add(grade);
		atoms.add(takes);
		atoms.add(takes);
		ProposedConjunction testClass=new ProposedConjunction(atoms);
        FeatureGenerator gen=new FeatureGenerator();
        Predicate fr=new BooleanPred("friend",2);
        Atom intelligence=new Atom(fr,new Logvar[]{student,student1});
       // Assert.assertEquals(1000,gen.createAllRenamingCombinations(Arrays.asList(new ProposedConjunction[]{testClass}), friendHead).size());
	    List<Conjunction> created_conjunctions=gen.createAllRenamingCombinations(Arrays.asList(new ProposedConjunction[]{testClass}), intelligence);
	   // Conjunction first=created_conjunctions.get(0);
	  //  Assert.assertEquals(first.getAtomList().get(0),first.getAtomList().get(1));
	    System.out.println(created_conjunctions);
	}
