...
public class TestProportionQuery {
	...
	public static void setUp() throws FeatureTypeException, ConjunctionConstructionProblem{
		...
		Standard_Conjunction conj=new Standard_Conjunction(difficulty,Arrays.asList(new PosLiteral(takes)));
	...
	public void testDataLoaderNoSelector() throws Exception{
		...
		System.out.println(new Theory(d.getInterpretations().get(0).getPrologFormat()));
		...
		NumberValue v=(NumberValue) tQrs.getFeatureValue(difficultyc1, engine,dep1.getFeatures().get(0), d.getInterpretations().get(0));
		assertEquals(75,v.getNumber(),0.01);
	}
	
	@Test
	public void testDataLoaderNoSelectorWithNegation() throws Exception{
		TuPrologDataLoader dataLoader=new TuPrologDataLoader(new TuPrologInterpretationCreator_Subsampling(1));
		Data d=dataLoader.loadData(pathToInterpretations, "interp", "pl", ntw);
		Prolog engine=new Prolog();
		InputStream is = this.getClass().getResourceAsStream("Queries.pl");
		...
		AbstractConjunction conj=new Standard_Conjunction(difficulty,Arrays.asList(new PosLiteral(takes)));
		//Dependency withNegation=dep1=Dependency(difficulty, new Feature[]{new Proportion(conj)});
		engine.addTheory(new Theory(d.getInterpretations().get(0).getPrologFormat()));
		Library lib=engine.loadLibrary("alice.tuprolog.lib.JavaLibrary");
		TuPrologQueries tQrs=new TuPrologQueries(lib);
		HashMap<Logvar,Argument> subst=new HashMap<Logvar, Argument>();
