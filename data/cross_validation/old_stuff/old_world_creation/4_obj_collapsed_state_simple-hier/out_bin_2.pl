...
move_left_of(W,Id,Id1) ~ finite([0.04106650321789764:true,0.9589334967821024:false]) := left_of(W,Id,Id1)~=B_M,B_M==true,shape(W,Id)~=Sh_M,Sh_M==square,shape(W,Id1)~=Sh_M_1,Sh_M_1==triangle.
move_left_of(W,Id,Id1) ~ finite([0.05821710127349909:true,0.9417828987265009:false]) := left_of(W,Id,Id1)~=B_M,B_M==true,shape(W,Id)~=Sh_M,Sh_M==square,shape(W,Id1)~=Sh_M_1,Sh_M_1==circle.
move_left_of(W,Id,Id1) ~ finite([0.11816367265469062:true,0.8818363273453094:false]) := left_of(W,Id,Id1)~=B_M,B_M==true,shape(W,Id)~=Sh_M,Sh_M==triangle.
move_left_of(W,Id,Id1) ~ finite([0.6374758531873793:true,0.36252414681262074:false]) := \+left_of(W,Id,Id1)~=B_M,shape(W,Id)~=Sh_M,Sh_M==square,shape(W,Id1)~=Sh_M_1,Sh_M_1==triangle.
move_left_of(W,Id,Id1) ~ finite([0.461271676300578:false,0.5387283236994219:true]) := \+left_of(W,Id,Id1)~=B_M,shape(W,Id)~=Sh_M,Sh_M==square,shape(W,Id1)~=Sh_M_1,Sh_M_1==circle.
move_left_of(W,Id,Id1) ~ finite([0.3816407661727503:true,0.6183592338272498:false]) := \+left_of(W,Id,Id1)~=B_M,shape(W,Id)~=Sh_M,Sh_M==triangle.
move_left_of(W,Id,Id1) ~ finite([0.23815824292140778:true,0.7618417570785923:false]).
displ(W,Id) ~ gaussian(Mean,0.05230536629799583) := move_left_of(W,Id_M,Id)~=true,true==true,posX_t0(W,Id)~=X_M,posX_t0(W,Id_M)~=X_M_1,getMean([X_M,X_M_1],[-0.9738547875487514,0.9317711896977967,-0.7890123486471534],Mean).
displ(W,Id) ~ gaussian(Mean,0.02188304567892457) := \+move_left_of(W,Id_M,Id)~=true,posX_t0(W,Id)~=X_M,getMean([X_M],[-0.013210571580732388,0.013298634627728116],Mean).
