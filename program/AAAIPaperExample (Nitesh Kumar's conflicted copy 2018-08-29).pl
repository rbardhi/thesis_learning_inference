...
intelligence(carl) ~ gaussian(4.8,0.1).
friend(X,Y) ~ finite([0.8:true, 0.2:false]) := takes(Z,X), takes(Z,Y), X\=Y.
date(X) ~ finite([Prob1:true, Prob2:false]) := findall_forward(I, (friend(X,Z)~=true, intelligence(Z)~=I), I1), avg(I1)~=Avg, logistic([2.0, -9.0], [Avg], Prob1), Prob2 is 1-Prob1.
...
test(N) :-
	init,
	query([date(ann)~=false],[],(friend(ann,bob)~=true),N,P),
